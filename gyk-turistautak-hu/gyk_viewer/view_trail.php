...
<body>
<h1>Jelzett útvonal - GyalogútKataszter v0.1</h1>
<span style="display: block; text-align: right;">
  <a href="../gyk_doc/doc_viewer_hu.html#view_trail" target="_blank"><img src="gui_img/question.png" border="0">&nbsp;Súgó</a>
</span>
<?

// main - print trail:
if (@$_REQUEST['trail_id'] == null) {
	printerr("Nincs megadva útvonal!");
} else {
	$trail = Trail::query_trail ($_REQUEST['trail_id']);
	if (@$trail == null) {
		printerr("A kért útvonal nem létezik vagy nem elérhető!");
	} else {
		print_trail_header ($trail);
                print_trail_sectionslink ($_REQUEST['trail_id']);
                print_trail_reverselink ($_REQUEST['trail_id']);
		$nodes = get_trail_nodes($_REQUEST['trail_id']);
		print_trail_nodetable ($nodes);
	}
}


function print_trail_header ($trail) {
...
function print_trail_sectionslink ($trail_id) {
    print("<span style=\"display: block; text-align: right;\"><a href=\"list_trail_sections.php?trail_id=" . $trail_id . "\">[Szakaszolás, paraméterek]</a></span><br>\n");
}

function print_trail_reverselink ($trail_id) {
    print("<span style=\"display: block; text-align: right;\"><a href=\"view_trail.php?trail_id=" . -$trail_id . "\">[Visszafelé irány]</a></span><hr>\n");
}

function get_trail_nodes ($trail_id) {
	$query = sprintf("SELECT * FROM trail_node_bidir JOIN noi ON trail_node_bidir.noi_id = noi.noi_id LEFT JOIN trail_branch ON trail_node_bidir.branch_id = trail_branch.branch_id WHERE trail_id = %s ORDER BY node_idx",
...
// list_trails.php-b�l �tv�ve - rendezend�, kell egy trail_view oszt�ly!
...
function print_trail_table ($trail_recordset) {
	print("<table border=1 width=\"100%\">\n");
	print_header_row ();
	while ($trail_record = mysql_fetch_assoc($trail_recordset)) {
		print_trail_row ($trail_record);
	}
	print("</table>\n");
}
